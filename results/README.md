...
This page is a reading guide to the completed study. It contains static figures reconstructed from the numerical results in the [main README](../README.md#7-what-this-repository-contains-today), so the figures can be read on GitHub without the private study website or disabled dashboard. The main README has the full tables, including all 20 individual training-start returns, and the corresponding machine-readable reports are available under [`data/`](data/).
